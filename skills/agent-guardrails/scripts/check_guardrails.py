...
BLOCKABLE_EVENTS = {"PreToolUse", "PostToolUse", "Stop", "SubagentStop", "UserPromptSubmit"}
        ...
        for gi, group in enumerate(groups):
            ...
            if not isinstance(group, dict) or "hooks" not in group:
                report(
                    "ERROR",
                    str(path),
                    f'{where} needs an inner "hooks" array (three-level nesting)',
                )
                continue
    ...
    for lineno, line in enumerate(text.splitlines(), 1):
        if re.search(r"\bexit 1\b", line) and re.search(r"block|deny|fail|refus", text, re.I):
            report(
                "WARN",
                str(script),
                f"line {lineno}: 'exit 1' does NOT block — only exit 2 does; anything else is logged and the action proceeds",
            )
            break
